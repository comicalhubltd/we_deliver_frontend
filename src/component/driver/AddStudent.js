...
const AddStudent = () => {
  ...
  async function fetchData() { 
    ...
    if (result.message === false) {
      setAlertType("error");
      setMessage(
        "No class found, provide classes for this school in the class section"
      );
      setOpen(true);
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    try {
      const result = await dispatch(saveDriver(values)).unwrap();
            ...
            >
              {({
                ...
                <Card>
                  ...
                  >
                    {isSubmitting ? "Submitting..." : "Save Student"}
                  </button>
                </Card>
              )}
            </Formik>

...
import {
  getPendingDelivery,  getDeliveredDelivery
...
const AcceptRequest = () => {
  ...
  const rows = Array.isArray(pendingDelivery) ? pendingDelivery : [];
  ...
  const navigateToRequest = (id) => {
    if (id !== 0){
      navigate(`/delivery/view-request-details/${initialSelectedId}`);
    } else {
      navigate(`/delivery/new-requests`);
    }
